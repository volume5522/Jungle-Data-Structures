...
/*
----------------------------------------------------------------------------------
문제 : 연결 리스트의 항목들을 스택에 복사하되, 리스트의 "첫 번째 노드부터 차례대로"
       push하여 결과적으로 스택에는 역순으로 저장되도록 해야 함.

예시 : LinkedList (10 -> 20 -> 30) 
      → push(10), push(20), push(30)
      → Stack (Top → 30, 20, 10)

- Stack은 LinkedList 기반이며, push는 항상 head에 삽입됨 (즉, index 0).
- 따라서 리스트를 앞에서부터 순차적으로 push하면 자동으로 Stack에 역순 저장됨.

절차:
1. 예외 처리 : ll이 NULL이거나 비어 있으면 return
2. 스택 초기화 (removeAllItemsFromStack)
3. 리스트를 순회하며 각 항목을 push
----------------------------------------------------------------------------------
*/
...
예시 : Stack (Top → 7, 6, 4, 3, 2) 
      → 제거 후 (Top → 7, 3)

핵심 아이디어:
- 스택은 LIFO 구조이므로 Top부터 pop해서 짝수는 제거하고,
  홀수만 임시 스택(temp)에 저장
- 이후 temp에서 다시 pop하며 원래 스택에 push
  → 이 과정을 통해 순서가 유지된 채 짝수만 제거됨

절차:
1. 예외 처리 : 스택이 NULL이거나 비어 있으면 return
2. 임시 스택(temp)을 생성하여 홀수만 저장
3. temp에서 다시 원래 스택으로 복원 (역순으로 다시 push)
----------------------------------------------------------------------------------
*/
